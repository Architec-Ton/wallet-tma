...
const encodeMnemonic = (mnemonic: string, salt: string) => {
  return CryptoES.TripleDES.encrypt(mnemonic, sha256_sync(salt).toString()).toString();
};

const decodeMnemonic = (hash: string, salt: string) => {
  return CryptoES.TripleDES.decrypt(hash, sha256_sync(salt).toString()).toString(CryptoES.enc.Utf8);
};
