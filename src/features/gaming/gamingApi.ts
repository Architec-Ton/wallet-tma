...
export const gamingApi = createApi({
	...
	endpoints: builder => ({
		...
		voteTxn: builder.mutation<{ success: boolean }, { appId: string; amount: number; comment: string }>({
			query: ({ appId, amount, comment }) => ({
				url: `https://dev-catalog.architecton.site/api/v2/catalog/app/${appId}/vote-txn`,
				method: 'POST',
				body: { amount, comment },
			}),
		}),
