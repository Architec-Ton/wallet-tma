...
const initialState = loadStateFromLocalStorage(SETTINGS_STORAGE_KEY) || createSettingsButtons()
...
const accountSettingsSlice = createSlice({
    ...
    reducers: {
        updateSettingValue: (state, action) => {
            const setting = state.find(setting => setting.name === action.payload.name);
            if (setting) {
