...
export class BankJetton implements Contract {
  ...
  async getCalculateStakeAddress(provider: ContractProvider, owner_address: Address) {
    let builder = new TupleBuilder();
    builder.writeAddress(owner_address);
    let source = (await provider.get("calculate_stake_address", builder.build())).stack;
