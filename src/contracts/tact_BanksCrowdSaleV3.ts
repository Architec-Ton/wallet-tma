...
import { Cell, Slice, TupleBuilder, beginCell, contractAddress } from "@ton/core";
...
function dictValueParserReferralAddress(): DictionaryValue<ReferralAddress> {
  return {
    ...
    parse: (src) => loadReferralAddress(src.loadRef().beginParse()),
  };
}
...
function dictValueParserSetBankOffset(): DictionaryValue<SetBankOffset> {
  return {
    ...
    parse: (src) => loadSetBankOffset(src.loadRef().beginParse()),
  };
}
...
function dictValueParserSetJettonWallet(): DictionaryValue<SetJettonWallet> {
  return {
    ...
    parse: (src) => loadSetJettonWallet(src.loadRef().beginParse()),
  };
}
...
function dictValueParserSetNewOwner(): DictionaryValue<SetNewOwner> {
  return {
    ...
    parse: (src) => loadSetNewOwner(src.loadRef().beginParse()),
  };
}
...
export class BanksCrowdSaleV3 implements Contract {
  ...
  async send(
    ...
    if (message && typeof message === "object" && !(message instanceof Slice) && message.$$type === "SetNewOwner") {
