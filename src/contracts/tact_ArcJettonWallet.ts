...
function initArcJettonWallet_init_args(src: ArcJettonWallet_init_args) {
  return (builder: Builder) => {
    let b_0 = builder;
    b_0.storeAddress(src.owner);
...
const ArcJettonWallet_errors: { [key: number]: { message: string } } = {
...
export class ArcJettonWallet implements Contract {
  static async init(owner: Address, jetton_master: Address) {
    return await ArcJettonWallet_init(owner, jetton_master);
  }

  static async fromInit(owner: Address, jetton_master: Address) {
    const init = await ArcJettonWallet_init(owner, jetton_master);
    const address = contractAddress(0, init);
    return new ArcJettonWallet(address, init);
  }

  static fromAddress(address: Address) {
    return new ArcJettonWallet(address);
  }

  readonly address: Address;
  readonly init?: { code: Cell; data: Cell };
  readonly abi: ContractABI = {
    types: ArcJettonWallet_types,
    getters: ArcJettonWallet_getters,
    receivers: ArcJettonWallet_receivers,
    errors: ArcJettonWallet_errors,
  };
