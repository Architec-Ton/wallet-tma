...
const AddCryptoAddress = () => {
  ...
  const copyAddressHandler = () => {
    if (tonState?.wallet) {
      navigator.clipboard.writeText(tonState.wallet).then(() => {
  ...
  return (
    <>
      <Section
        title={t("your-coin-address", undefined, { coin: tonState?.coin })}
        className="address-description__container"
      >
      ...
      <Section title="" className="qrcode-section">
        <Block className="qrcode-block">
          <Column className="address-qrcode">
            <QRCode value={`ton://transfer/${tonState?.wallet}`} size={190} logoImage={tonState?.thumb} />
            <div className="qrcode-description">{t("qr-hint", undefined, { title: tonState?.title })}</div>
          </Column>
        </Block>
        <Block>
          <Column className="wallet-info">
            ...
            <button className="primary-button wallet-copy-button" onClick={copyAddressHandler}>
              {t("copy")}
            </button>
          </Column>
        </Block>
