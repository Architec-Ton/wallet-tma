  ...
  return (
    <Section
      ...
      readMore={`${t("balance")}: 
        ${
          (coin &&
            (Number(coin?.amount) >= Number(assetValue) ? Number(coin?.amount) - Number(assetValue) : 0).toLocaleString(
              undefined,
              { maximumFractionDigits: coin?.meta?.decimals },
            )) ||
          0
        }`}
    >
      <Row className="justify-between asset-row">
        ...
        <Row className="controls-container">
          <button className="rounded-button control-button" onClick={clearHandler} disabled={!asset.title}>
            {t("clear")}
          </button>
          <button
            className="rounded-button control-button"
            onClick={halfHandler}
            disabled={!asset.title || isInsuffucientBalance}
          >
            50%
          </button>
          <button
      ...
      <Row className="justify-between asset-data-row">
        ...
        <div className="asset-feat-value">
          {coin &&
            (coin.usdPrice * Number(assetValue)).toLocaleString(undefined, { style: "currency", currency: "USD" })}
        </div>
      </Row>
    </Section>
  );
